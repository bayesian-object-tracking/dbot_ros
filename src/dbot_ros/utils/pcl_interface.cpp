...
#include <dbot_ros/utils/cloud_visualizer.hpp>

namespace pi
{
template <typename PointT, typename MatrixT>
void Pcl2Eigen(const pcl::PointCloud<PointT>& point_cloud,
...
template <typename T>
void Ros2Std(const sensor_msgs::PointCloud2& ros_cloud,
             ...
             size_t& n_cols)
{
    ...
    n_rows = point_cloud->height / n_downsampling;
    n_cols = point_cloud->width / n_downsampling;
    observations.resize(n_rows * n_cols);
    for (size_t row = 0; row < n_rows; row++)
        for (size_t col = 0; col < n_cols; col++)
            observations[row * n_cols + col] =
                point_cloud->at(col * n_downsampling, row * n_downsampling).z;
}

template <typename PointT, typename MatrixT>
std::vector<Eigen::Matrix<MatrixT, 3, 1>> Pcl2Eigen(
...
template <typename PointT, typename MatrixT>
void Eigen2Pcl(const std::vector<Eigen::Matrix<MatrixT, 3, 1>>& vector,
               const size_t& n_rows,
               const size_t& n_cols,
...
template <typename PointT, typename MatrixT>
void PointsOnPlane(const pcl::PointCloud<PointT>& input_cloud,
                   ...
                   const float& min_table_dist = 0.01,
                   const float& max_table_dist = 0.4,
                   const float& grid_size = 0.02)
{
    ...
    boost::shared_ptr<pcl::PointCloud<PointT>> temp_cloud(
        new pcl::PointCloud<PointT>);
    *temp_cloud = input_cloud;

    boost::shared_ptr<pcl::PointCloud<PointT>> input_cloud_ptr(
        new pcl::PointCloud<PointT>);
    ...
    *temp_cloud = downsampled;
    ...
    // Estimate normals ----------------------------------------
    boost::shared_ptr<pcl::search::KdTree<PointT>> normals_tree_(
    ...
    // -----------------------------------------
    boost::shared_ptr<pcl::PointCloud<PointT>> table_projected(
        new pcl::PointCloud<PointT>);
    pcl::ProjectInliers<PointT> proj_;
    proj_.setModelType(pcl::SACMODEL_PLANE);
    proj_.setInputCloud(temp_cloud);
    const pcl::PointIndices::ConstPtr table_inliers_const(
        table_inliers);  // just to make eclipse shut up
    ...
    // ---------------------------------------------------------
    boost::shared_ptr<pcl::PointCloud<PointT>> table_hull(
        new pcl::PointCloud<PointT>);
...
void Cluster(const pcl::PointCloud<PointT>& input_point_cloud,
             ...
             const float& erosion_pixel_radius = 2)
{
    ...
    {
        std::cout << "for clustering point cloud has to be organized"
                  << std::endl;
        exit(-1);
    }
    ...
    for (int col = -int(erosion_pixel_radius); col <= int(erosion_pixel_radius);
         col++)
        for (int row = -int(erosion_pixel_radius);
             row <= int(erosion_pixel_radius);
             row++)
    ...
    for (unsigned int col = 0; col < point_cloud.width; col++)
        for (unsigned int row = 0; row < point_cloud.height; row++)
        {
            ...
            for (int col_ = int(col - 1); col_ <= int(col + 1); col_++)
                ...
                {
                    if (col_ < 0 || col_ >= (int)point_cloud.width ||
                        row_ < 0 || row_ >= (int)point_cloud.height)
                    {
                    ...
                    Eigen::Vector3f point =
                        point_cloud.at(col_, row_).getVector3fMap();
                    Eigen::Vector3f center =
                        point_cloud.at(col, row).getVector3fMap();
                    if ((point - center).norm() > max_depth ||
                        isnan(point_cloud.at(col_, row_).x))
                    {
                        bad_indices.push_back(Eigen::Vector2i(col, row));
                        goto end;
                    }
    ...
    boost::shared_ptr<pcl::PointCloud<PointT>> eroded_cloud(
        new pcl::PointCloud<PointT>);
    *eroded_cloud = point_cloud;
    for (size_t i = 0; i < bad_indices.size(); i++)
    {
        for (size_t j = 0; j < erosion_operator.size(); j++)
        {
            ...
            if (col_ < 0 || col_ >= (int)eroded_cloud->width || row_ < 0 ||
                row_ >= (int)eroded_cloud->height ||
                isnan(eroded_cloud->at(col_, row_).x))
                continue;

            eroded_cloud->at(col_, row_).getVector3fMap() =
    ...
    for (unsigned int col = 0; col < point_cloud.width; col++)
        for (unsigned int row = 0; row < point_cloud.height; row++)
        {
            ...
            while (indices.size())
            {
                ...
                for (int window_col = center_col - window;
                     window_col < center_col + window;
                     window_col++)
                    for (int window_row = center_row - window;
                         window_row < center_row + window;
                         window_row++)
                    {
                        if (window_col < 0 ||
                            window_col >= (int)point_cloud.width ||
                            window_row < 0 ||
                            window_row >= (int)point_cloud.height ||
                            isnan(point_cloud.at(window_col, window_row).x))
                            continue;

                        Eigen::Vector3f point =
                            point_cloud.at(window_col, window_row)
                                .getVector3fMap();

                        if ((center - point).norm() <= cluster_delta_)
                        {
                            if (!isnan(
                                    eroded_cloud->at(window_col, window_row).x))
                            {
                            ...
                            indices.push_back(
                                Eigen::Vector2i(window_col, window_row));
                            centers.push_back(
                                point_cloud.at(window_col, window_row)
                                    .getVector3fMap());
                            point_cloud.at(window_col, window_row)
    ...
    const float& erosion_pixel_radius = 2)
{
    ...
    Cluster(input_cloud,
            ...
            cluster_delta,
            min_cluster_size,
            erosion_pixel_radius);

    output_points.resize(output_cloud.size());
    for (size_t i = 0; i < output_cloud.size(); i++)
        output_points[i] =
            pi::Pcl2Eigen<pcl::PointXYZ, MatrixT>(output_cloud[i]);
}

template <typename PointT>
void FindCylinder(const pcl::PointCloud<PointT>& input_point_cloud,
                  ...
                  const float r_max)
{
    ...
    *point_cloud_ptr = input_point_cloud;
    ...
    boost::shared_ptr<pcl::ModelCoefficients> coefficients_cylinder(
    ...
    boost::shared_ptr<pcl::PointCloud<PointT>> inliers_ptr(
        new pcl::PointCloud<PointT>);
    *inliers_ptr = inliers;
...
std::vector<osr::FreeFloatingRigidBodiesState<-1>::State> SampleTableClusters(
    ...
    const size_t& sample_count)
{
    ...
    if (clusters.size() == 0) return states;
         ...
         cluster_index++)
    {
        ...
        Vector t_mean =
            com -
            (com.dot(table_normal) + table_plane(3)) *
                table_normal;  // project center of mass in table plane
        Vector table_vector_a = table_normal.cross(Vector(1, 1, 1))
                                    .normalized();  // vector along table plane
        Vector table_vector_b = table_normal.cross(
            table_vector_a);  // second vector along table plane
        Matrix R_mean;
        R_mean.col(0) = table_vector_a;
        R_mean.col(1) = table_vector_b;
        R_mean.col(2) = table_normal;
        ...
        for (size_t i = 0; i < size_t(sample_count) / clusters.size(); i++)
        {
            ...
            body_system.component(0).pose().position() =
                t_mean +
                standard_deviation_translation * unit_gaussian.sample()(0) *
                    table_vector_a +
                standard_deviation_translation * unit_gaussian.sample()(0) *
                    table_vector_b;
            body_system.component(0).orientation().quaternion(
                Eigen::Quaterniond(
                    Eigen::AngleAxisd(
                        standard_deviation_rotation * unit_gaussian.sample()(0),
                        table_normal) *
                    R_mean));
...
template <typename Scalar>
std::vector<osr::FreeFloatingRigidBodiesState<-1>::State> SampleTableClusters(
    const Eigen::Matrix<Eigen::Matrix<Scalar, 3, 1>, -1, -1>& points,
    const size_t& sample_count)
{
    std::vector<Eigen::Matrix<Scalar, 3, 1>> std_points(points.rows() *
                                                        points.cols());
    for (size_t row = 0; row < points.rows(); row++)
        for (size_t col = 0; col < points.cols(); col++)
            std_points[row * points.cols() + col] = points(row, col);

    return SampleTableClusters(
        std_points, points.rows(), points.cols(), sample_count);
}
}

template std::vector<osr::FreeFloatingRigidBodiesState<-1>::State>
pi::SampleTableClusters(
    const Eigen::Matrix<Eigen::Matrix<double, 3, 1>, -1, -1>&,
    const size_t&);
...
template void pi::PointsOnPlane(
...
template void pi::Pcl2Eigen(const pcl::PointCloud<pcl::PointXYZ>& point_cloud,
                            std::vector<Eigen::Matrix<double, 3, 1>>& vector,
                            size_t& n_rows,
                            size_t& n_cols);

template std::vector<Eigen::Matrix<double, 3, 1>> pi::Pcl2Eigen(
    const pcl::PointCloud<pcl::PointXYZ>& point_cloud);

template void pi::Eigen2Pcl(
